...
import { gradeCardTranslation } from "$lib/features/lessons/utilities"
...
export const POST = async ({ request, locals: { supabase, getSession }}) => {
  ...
  try {
    ...
    }, error: cardGradeError } = gradeCardTranslation({
      card: sentenceCard,
      transcription,
      confidence,
    })
    ...
    const review =  card.reviews ? card.reviews.find(({ type }) => type === 'SPEAK') : null
